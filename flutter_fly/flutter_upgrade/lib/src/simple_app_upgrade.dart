...
class _SimpleAppUpgradeWidget extends State<SimpleAppUpgradeWidget> {
  ...
  _clickOk() async {
    if (Platform.isIOS) {
      ...
      if(widget.iosAppId==null || widget.iosAppId.isNotEmpty){
        Navigator.of(context).pop();
        showDialog(context: context,builder: (context){
          return AlertDialog(
            shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
            content: Text('当前App未上传到App Store，请到Github更新源代码'),
            actions: <Widget>[
              FlatButton(child: Text('我知道了'),onPressed: (){Navigator.of(context).pop();},),
            ],
          );
        });
        return;
